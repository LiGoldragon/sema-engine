...
`Match`, typed `ReadPlan` vocabulary, `SnapshotId`, operation-log replay,
`list_tables`, and the first `Subscribe` primitive with durable registration,
initial snapshot, and post-commit delta delivery. Executable read plans now
cover all rows, exact key, and key range. Query-algebra operators
