...
fn versioned_commit_log_carries_payloads_and_digest_chain() {
    ...
    assert_eq!(first_operation.key().map(RecordKey::to_owned_string).as_deref(), Some("alpha"));
    assert_eq!(
    ...
    assert_eq!(second_operation.key().map(RecordKey::to_owned_string).as_deref(), Some("alpha"));
...
fn versioned_commit_log_preserves_atomic_operation_bundle() {
    ...
    assert_eq!(
        log[0].operations().head().key().map(RecordKey::to_owned_string).as_deref(),
        Some("alpha")
    );
    assert_eq!(
        log[0].operations().tail()[0].key().map(RecordKey::to_owned_string).as_deref(),
        Some("beta")
    );
    assert_eq!(
...
fn replay_from_sequence_uses_durable_commit_sequence_cursor() {
    ...
    assert_eq!(
        replay[0].operations().head().key().map(RecordKey::to_owned_string).as_deref(),
        Some("beta")
    );
}
...
fn versioned_replay_from_sequence_uses_the_same_inclusive_cursor() {
    ...
    assert_eq!(
        replay[1].operations().head().key().map(RecordKey::to_owned_string).as_deref(),
        Some("gamma")
    );
